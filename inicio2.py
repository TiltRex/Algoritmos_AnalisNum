...
def bisection_method(a, b, max_iter, max_error_percentage):
    def user_function(c):
        return ((68.1 * 9.8) / c) * (1 - math.e**((-c / 68.1) * 10)) - 40
...
def false_position_method(a, b, max_iter, max_error_percentage):
    def user_function(c):
        return ((68.1 * 9.8) / c) * (1 - math.e**((-c / 68.1) * 10)) - 40
...
def fixed_point_method(c0, max_iter, max_error_percentage):
    x = sp.symbols("x")

    def user_function_f(x):
        return ((68.1 * 9.8) / x) * (1 - math.e**((-x / 68.1) * 10)) - 40

    f = user_function_f(x)
    df = sp.diff(f, x)

    for i in range(max_iter):
        f_c = user_function_f(c0)  # Calcular f(c)
        g_c = c0 - f_c / df.subs(x, c0)  # Calcular g(c) utilizando la derivada
        error_percentage = abs((g_c - c0) / g_c) * 100  # Calcular el porcentaje de error
        if error_percentage < max_error_percentage:
...
def newton_raphson_method(initial_guess, max_iterations, max_error_percentage):
    x = sp.symbols('x')
    ...
    for i in range(max_iterations):
        f_value = user_function(c)
        f_derivative = sp.diff(user_function(x), x).subs(x, c)
...
def secant_method(initial_guess_1, initial_guess_2, max_iterations, max_error_percentage):
    x = sp.symbols('x')
    ...
    for i in range(max_iterations):
        ...
        f_c0 = user_function(c0)
        f_c1 = user_function(c1)
...
while flag == False:
    ...
    """
        ...
        a = 10  # Establece el valor del intervalo a
        b = 20  # Establece el valor del intervalo b
        ...
        approximate_c, num_iterations = bisection_method(a, b, max_iterations, max_error_percentage)
    ...
    """
        ...
        approximate_c, num_iterations = false_position_method(a, b, max_iterations, max_error_percentage)
    ...
    """
        ...
        # Valor inicial c0
        c0 = 10  # Establece el valor inicial de aproximación
        ...
        approximate_solution, num_iterations = fixed_point_method(
            c0, max_iterations, max_error_percentage
        )
    ...
    """
        ...
        # Valor inicial c0
        initial_guess = 10  # Establece el valor inicial de aproximación
        ...
        approximation, num_iterations = newton_raphson_method(initial_guess, max_iterations, max_error_percentage)
    ...
    """
        ...
        initial_guess_1 = 10  # Establece el valor del intervalo a
        initial_guess_2 = 20  # Establece el valor del intervalo b
        ...
        approximation, num_iterations = secant_method(initial_guess_1, initial_guess_2, max_iterations, max_error_percentage)
